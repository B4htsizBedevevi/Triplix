...
const worlds = {
  Meyve:{emoji:'',title:'Meyve Bahçesi',tagline:'Tatlı eşleşmeler, büyük combo!',icons:pack.fruit,colors:['#ef6680','#f7bb50','#63cf7d','#4faee9','#a37cf1','#ec8b59'],bg:'orchard'},
  Kristal:{emoji:'',title:'Kristal Dünyası',tagline:'Parla, eşleş, patla!',icons:pack.crystal,colors:['#4baeff','#5978ff','#55d59d','#ad66ff','#ff6885','#e7be51'],bg:'crystal'},
  Sihir:{emoji:'',title:'Sihirli Objeler',tagline:'Küçük detaylar, büyük maceralar!',icons:pack.magic,colors:['#9b70ef','#5e9dff','#ce6fe0','#78dbcf','#dd9b5d','#8784dc'],bg:'magic'},
  Kozmik:{emoji:'',title:'Kozmik Serüven',tagline:'Uzayın en parlak üçlüleri',icons:pack.space,colors:['#4ca8ff','#7e78ff','#6a9aff','#69d5ac','#ffc857','#ba86ff'],bg:'space'},
  Dostlar:{emoji:'',title:'Sevimli Dostlar',tagline:'Tatlı karakterler, renkli eşleşmeler',icons:pack.friends,colors:['#ef8ea5','#b7a57e','#7e99cf','#af81e2','#e97878','#75c99e'],bg:'friends'},
  Antik:{emoji:'',title:'Antik Semboller',tagline:'Zamanı aşan üçlüler',icons:pack.crystal,colors:['#d5ac58','#6c98c7','#d7b76c','#8c7454','#63a27c','#b88655'],bg:'ancient'},
  Dogal:{emoji:'',title:'Doğa Elementleri',tagline:'Doğanın gücü seninle',icons:pack.magic,colors:['#63d17c','#4caee4','#ed6d5e','#7e9ef0','#9d8465','#e18bb1'],bg:'nature'},
  Mevsim:{emoji:'',title:'Mevsimler',tagline:'Her mevsim farklı bir güzellik',icons:pack.fruit,colors:['#71aef0','#ee9ab9','#ffc85a','#e18a67','#c87957','#83a8d9'],bg:'season'},
  Koleksiyon:{emoji:'',title:'Özel Koleksiyon',tagline:'Sadece TRIPLIX’e özel!',icons:pack.crystal,colors:['#e0b74f','#e66f85','#57a9ed','#62ca90','#f4ba4f','#9b7be9'],bg:'collection'}
} as const
...
function GameIcon({icon}:{icon:any}){
  return <Icon icon={icon} className="game-icon" aria-hidden="true"/>
}
...
function App(){
  ...
  function selectStone(stone:Stone){
    ...
    window.setTimeout(()=>{setBoard(cur=>cur.filter(x=>x.id!==stone.id));setTray(cur=>[...cur,stone]);setMoves(v=>Math.max(0,v-1));setSelectedId(null);setMessage('Taş tepsiye gidiyor…')},170)
  }
  ...
  return <main className={`app-shell world-${theme.bg}`}>
    ...
    <header className="topbar">
      <button className="brand" onClick={()=>setScreen('home')}><span className="brand-mark">{world==='Kozmik'?'◈':'◆'}</span><b>TRIPLIX</b></button>
    ...
    {screen==='home'&&<section className="home">
      <div className="hero-world">
        <div className="hero-vignette"/><div className="hero-copy">
          <span className="kicker">TRIPLIX WORLDS</span>
          <h1>Bir Taştan<br/><em>Daha Fazlası!</em></h1>
          <p>Farklı dünyaları keşfet. Taşları seç, üçlüleri patlat, combo'yu büyüt.</p>
          <div className="home-buttons"><button className="primary" onClick={()=>setScreen('worlds')}>DÜNYANI SEÇ <b>→</b></button><button className="ghost" onClick={()=>start(1,world)}>HEMEN OYNA</button></div>
        </div>
        <div className="showcase-stones"><div className="show-stone ss1"><GameIcon icon={theme.icons[0]}/></div><div className="show-stone ss2"><GameIcon icon={theme.icons[1]}/></div><div className="show-stone ss3"><GameIcon icon={theme.icons[2]}/></div><div className="show-stone ss4"><GameIcon icon={theme.icons[3]}/></div></div>
      </div>
