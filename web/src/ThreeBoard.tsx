...
const tileSize=1.02,gap=1.13,boardZ=(y:number)=>3-y
const pos=(s:ThreeStone)=>new THREE.Vector3((s.x-3)*gap,.58+s.layer*.24,boardZ(s.y)*gap)
...
export default function ThreeBoard({stones,tray,theme,selectedId,fx,onPick}:Props){
 ...
 useEffect(()=>{
  ...
  const floor=new THREE.Mesh(new RoundedBoxGeometry(11,.7,11,5,.35),new THREE.MeshStandardMaterial({color:new THREE.Color(theme.table),roughness:.72,metalness:.08}));floor.position.y=-.45;floor.receiveShadow=true;scene.add(floor)
  ...
  const burst=new THREE.Group();burstRef.current=burst;scene.add(burst)
  let px=0,pz=0
  const pointer=(e:PointerEvent)=>{const r=el.getBoundingClientRect();px=(e.clientX-r.left)/r.width*2-1;pz=(e.clientY-r.top)/r.height*2-1}
  ...
  const tick=()=>{raf.current=requestAnimationFrame(tick);const now=performance.now()/1000;camera.position.x+=(px*.75-camera.position.x)*.035;camera.position.z+=(11.8+pz*.55-camera.position.z)*.035;camera.lookAt(0,.25,0);meshRef.current.forEach(m=>{const selected=m.userData.selected as boolean,target=m.userData.flightTarget as THREE.Vector3|undefined;if(selected&&target){m.position.lerp(target,.13);m.rotation.y+=.055;m.rotation.z=Math.sin(now*10)*.06}else if(!selected){m.rotation.y+=(Math.sin(now+m.userData.phase)*.0015-m.rotation.y)*.08}});burst.children.forEach(p=>{const v=p.userData.v as THREE.Vector3;p.position.add(v);v.y-=.006;(p.material as THREE.MeshBasicMaterial).opacity*=.965});renderer.render(scene,camera);const next:Record<number,{x:number;y:number;z:number;scale:number}>={};meshRef.current.forEach((m,id)=>{const p=m.position.clone().add(new THREE.Vector3(0,.62,0));p.project(camera);const r=renderer.domElement.getBoundingClientRect();next[id]={x:(p.x*.5+.5)*r.width,y:(-p.y*.5+.5)*r.height,z:p.z,scale:Math.max(.7,Math.min(1.3,12/(m.position.distanceTo(camera.position)+.01)))} });setPoints(next)};tick()
  return()=>{cancelAnimationFrame(raf.current);el.removeEventListener('pointermove',pointer);ro.disconnect();renderer.dispose();el.removeChild(renderer.domElement);scene.clear();meshRef.current.clear()}
 },[theme.table,theme.accent])
 useEffect(()=>{const scene=sceneRef.current;if(!scene)return;const old=[...meshRef.current.values()];old.forEach(m=>{scene.remove(m);m.geometry.dispose();(m.material as THREE.Material).dispose()});meshRef.current.clear();stones.forEach((s,i)=>{const geo=new RoundedBoxGeometry(tileSize,.42,tileSize,5,.13);const mat=new THREE.MeshStandardMaterial({color:worldColors[s.kind%worldColors.length],roughness:.32,metalness:.12});const m=new THREE.Mesh(geo,mat);m.position.copy(pos(s));m.castShadow=true;m.receiveShadow=true;m.userData={id:s.id,selected:s.id===selectedId,phase:i*.37};m.rotation.y=(s.id%7)*.06;scene.add(m);meshRef.current.set(s.id,m)})},[stones,selectedId,worldColors])
 ...
 useEffect(()=>{const group=burstRef.current;if(!group||!fx)return;group.clear();for(let i=0;i<28;i++){const a=(i/28)*Math.PI*2,r=.04+Math.random()*.13,g=new THREE.SphereGeometry(.035+Math.random()*.045,5,5),m=new THREE.MeshBasicMaterial({color:new THREE.Color(theme.accent),transparent:true,opacity:1}),p=new THREE.Mesh(g,m);p.position.set((Math.random()-.5)*.35,1+(Math.random()-.5)*.25,(Math.random()-.5)*.35);p.userData.v=new THREE.Vector3(Math.cos(a)*r,.04+Math.random()*.11,Math.sin(a)*r);group.add(p)}const timer=window.setTimeout(()=>group.clear(),760);return()=>clearTimeout(timer)},[fx,theme.accent])
